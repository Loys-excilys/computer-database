	...
	public void doTag() throws JspException {
		try {
			...
			if (this.computerFormAddDTO != null) {
				this.getJspContext().getOut()
						.println(COMPUTER_NAME + "value=\"" + this.computerFormAddDTO.getName() + INTRODUCED_DATE
								+ this.computerFormAddDTO.getIntroduced() + DISCONTINUED_DATE
								+ this.computerFormAddDTO.getDiscontinued());
			} else {
				this.getJspContext().getOut().println(COMPUTER_NAME + INTRODUCED_DATE + DISCONTINUED_DATE);
			}
			this.getJspContext().getOut().println(
					"\">" + "</div>" + "<div class=\"form-group\">" + "<label for=\"companyId\">Company</label>"
							+ "<select class=\"form-control\" id=\"companyId\" name=\"companyName\">"
							+ "<option value=\"\">---</option>");
			for (CompanyDTO company : this.listCompany) {
				if (this.computerFormAddDTO != null
						&& this.computerFormAddDTO.getCompanyId().compareTo(String.valueOf(company.getId())) == 0) {
